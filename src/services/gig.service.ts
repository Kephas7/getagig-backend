...
export class GigService {
    ...
    async updateGig(userId: string, gigId: string, data: UpdateGigDto): Promise<GigResponseDto> {
        const organizer = await this.organizerRepository.findByUserId(userId);
        if (!organizer) {
            throw new HttpError(403, "Unauthorized");
        }

        const gig = await this.gigRepository.update(gigId, organizer._id.toString(), data);
        if (!gig) {
    ...
    async deleteGig(userId: string, gigId: string): Promise<void> {
        ...
        const gig = await this.gigRepository.delete(gigId, organizer._id.toString());
        if (!gig) {
            throw new HttpError(404, "Gig not found or unauthorized");
        }
    }
