...
export class DashboardService {
  ...
  async getMusicianDashboardStats(userId: string) {
    ...
    const confirmedGigs = applications.filter(
    ...
    const recentGigs = applications
      ...
      .map((app) => {
        ...
        return {
          id:
            gig?._id?.toString() || app.gigId?.toString() || app._id.toString(),
          title: gig?.title || "Unknown Gig",
          status: app.status.charAt(0).toUpperCase() + app.status.slice(1),
          date: app.createdAt
            ? app.createdAt.toLocaleDateString()
            : new Date().toLocaleDateString(),
          location: gig?.location || "Unknown Location",
        };
      });
  ...
  async getOrganizerDashboardStats(userId: string) {
    ...
    for (const gig of gigs) {
      const apps = await this.applicationRepository.findByGigId(
        gig._id.toString(),
      );
      ...
      apps.forEach((app) => {
        recentApplications.push({
          ...
          status: app.status.charAt(0).toUpperCase() + app.status.slice(1),
          musicianId: (
            (app as any).musicianId?._id || app.musicianId
          ).toString(),
        });
      });
    }
