...
import { ApplicationResponseDto, CreateApplicationDto } from "../dtos/application.dto";
...
export class ApplicationService {
    ...
    async applyToGig(musicianId: string, data: CreateApplicationDto): Promise<ApplicationResponseDto> {
        // Check if gig exists
        ...
        const existing = await this.applicationRepository.findOne({ gigId: data.gigId, musicianId });
        if (existing) {
            throw new HttpError(400, "You have already applied for this gig");
        }

        const application = await this.applicationRepository.create(musicianId, data);

        // Send notification to organizer
        try {
            const musician = await MusicianModel.findById(musicianId);
            const organizerUserId = (gig.organizerId as any)?.userId?._id || (gig.organizerId as any)?.userId;
    ...
    async getApplicationsByGig(gigId: string, organizerId: string): Promise<ApplicationResponseDto[]> {
        ...
        const gigOrganizerId = (gig.organizerId as any)._id?.toString() || gig.organizerId.toString();
    ...
    async getMusicianApplications(musicianId: string): Promise<ApplicationResponseDto[]> {
        const applications = await this.applicationRepository.findByMusicianId(musicianId);
        return applications.map(app => this.toResponseDto(app));
    }

    async updateApplicationStatus(id: string, organizerId: string, status: "accepted" | "rejected"): Promise<ApplicationResponseDto> {
        ...
        try {
            const musicianUserId = (application.musicianId as any)?.userId?._id || (application.musicianId as any)?.userId;
            if (musicianUserId) {
    ...
    private toResponseDto(app: IApplication): ApplicationResponseDto {
        ...
        return {
            ...
            musician: (musician && (musician.stageName || musician.userId)) ? {
                id: (musician._id || musician).toString(),
                username: musician.userId?.username || musician.stageName || "Unknown Musician",
