...
describe("Gig Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    await UserModel.deleteMany({
      email: { $in: [testOrganizerUser.email, testMusicianUser.email] },
    });
    ...
    await request(app)
      ...
      .send({
        ...testOrganizerUser,
        confirmPassword: testOrganizerUser.password,
      });
    ...
    await request(app)
      ...
      .send({
        ...testMusicianUser,
        confirmPassword: testMusicianUser.password,
      });
    const musLogin = await request(app).post("/api/auth/login").send({
      email: testMusicianUser.email,
      password: testMusicianUser.password,
    });
  ...
  describe("GET /api/gigs", () => {
    ...
    test("should filter gigs by location", async () => {
      const response = await request(app).get("/api/gigs?location=New York");
      expect(response.status).toBe(200);
      expect(response.body.data.gigs[0].location).toContain("New York");
    });
  });
