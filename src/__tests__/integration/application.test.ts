...
describe("Application Integration Tests", () => {
    ...
    beforeAll(async () => {
        await mongoose.connect(process.env.MONGODB_URI_TEST || "mongodb://localhost:27017/getagig-test");
        ...
        musicianId = (musician._id as any).toString();
        musicianToken = jwt.sign({ id: musicianUser._id, role: "musician" }, JWT_SECRET, { expiresIn: "1h" });
        ...
        organizerToken = jwt.sign({ id: organizerUser._id, role: "organizer" }, JWT_SECRET, { expiresIn: "1h" });
        ...
        // Verification
        const checkMusician = await MusicianModel.findOne({ userId: musicianUser._id });
        if (!checkMusician) throw new Error("Musician creation failed in test setup");
