...
describe("Organizer Integration Tests", () => {
    ...
    beforeAll(async () => {
        await UserModel.deleteMany({ email: { $in: [organizerUser.email, musicianUser.email] } });
    ...
    describe("POST /api/organizers/profile", () => {
        ...
        test("should create an organizer profile", async () => {
            ...
            expect(response.body.data).toHaveProperty("organizationName", profileData.organizationName);
        });
        ...
        test("should not allow musician to create an organizer profile", async () => {
            ...
            expect(response.body.message).toBe("Forbidden: This action requires one of the following roles: organizer");
        });
    ...
    describe("PUT /api/organizers/profile", () => {
        test("should update own profile", async () => {
            ...
            expect(response.status).toBe(200);
            expect(response.body.success).toBe(true);
            expect(response.body.data).toHaveProperty("organizationName", updateData.organizationName);
        });
    });
});
